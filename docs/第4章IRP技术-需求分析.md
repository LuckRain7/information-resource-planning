...
一级数据流程图（1-DFD ）是建立业务模型、 调查记录某一职能域的内外信息流情况的手段。 结合用户视图的定义， 复查一级数据流程图， 记录每个职能域的输出、 存储和输人数据流， 保证全企业的数据流的一致性， 是重要的数据分析工作。
...
1. <span style="color:red;">输入、输出数据流</span>

输人、输出数据流是指职能域内外的数据流，一般有两种情况：
...
每一数据流标准统计期的数据流量＝生存期换算系数 x 记录数 x 数据元素数 x 10 x 统计参数
